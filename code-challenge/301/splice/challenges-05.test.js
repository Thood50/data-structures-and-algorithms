...
const listFoods = (recipe) => {
  let result = [];
  let regex = /^(?:\w+\s+){2}(.+)/g;
  recipe.forEach((value, idx, arr) => {
    let match = recipe.match(regex);
    console.log(match);
    result.push(match);
  })
  return result;
...
const splitFoods = (recipe) => {
  let result = [];
  let theRecipe = recipe.ingredients;
  let word = theRecipe.Split(' ');
  result.push(`word[2] word[3] word[4]`);
  return result;
}
...
const stepActions = (recipe) => {
  ...
  result.push(recipe.steps.match(regex));
  return result;
}

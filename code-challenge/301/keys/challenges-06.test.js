...
const hasChildrenValues = (arr, character) => {
    let childs;
    Object.values(arr).forEach(something => {
     if (something.name === character) {
        childs = something.children.length > 0 ? true:false;
...
const hasChildrenEntries = (arr, character) => {
    let childs;
    Object.entries(arr).forEach(something => {
     if (something.name === character) {
        childs = something.children.length > 0 ? true:false;
...
const houseSize = (arr) => {
  const sizes = [];
  function Greathouse(house, members) {
      this.house
  }
  return sizes;
}

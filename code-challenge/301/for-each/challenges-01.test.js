...
const createList = (availableItems) => {
  let instock = [];
  availableItems.forEach( (value, idx, availableItems) => {
      if (availableItems[idx].available === true) {
          instock.push(availableItems[idx].name);
      }
  });
  return instock;
...
const fizzbuzz = (arr) => {
  let newArray = [];
  let both = false
  arr.forEach ( (value, idx, arry) => {
      if ((value % 3) === 0 && (value % 5) === 0) {
        newArray.push('Fizz Buzz');
        both = true;
      } else if ((value % 3) === 0 && both === false) {
        newArray.push('Fizz');
      } else if ((value % 5) === 0 && both === false) {
        newArray.push('Buzz');
      } else {
        newArray.push(value);  
      }

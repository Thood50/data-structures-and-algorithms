...
////////////// Best Guess /////////////
  ...
  arr.forEach((value, index, array) => {
      if(arr[index].name !== arr[0].name) {
          if(arr[index].mass > arr[0].mass) {
              newArray.push(arr[index].name);
          }
      }
  })
...
const detectTicTacToeWin = (board) => {
    ...
    board.forEach((value, index, array) => {
        board[index].forEach((val, idx, arr) => {
            if(board[index][idx] === 'X' || board[index][idx] === 'O' ) {
                if(idx === 0) {

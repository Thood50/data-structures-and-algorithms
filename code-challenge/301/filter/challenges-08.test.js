...
const oddValues = (arr) => {
  let newArray;
  newArray = arr.filter(num => (num % 2) === 1);
  return newArray;
};
...
const filterStringsWithVowels = (arr) => {
  let newArray;
  let regex = /\w+[aeiou]\w+/g;
  newArray = arr.filter(word => {
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArray;
  newArray = arr.filter((word, idx) => word[idx] === forbiddenValues[idx]);
  return newArray;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArray;
  newArray = arr.filter(obj => obj.baseStat > minBaseStat);
  return newArray;
};
